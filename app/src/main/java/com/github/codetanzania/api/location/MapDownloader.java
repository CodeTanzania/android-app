package com.github.codetanzania.api.location;
...
public class MapDownloader implements OfflineManager.CreateOfflineRegionCallback {
    private static final String JSON_FIELD_REGION_NAME = "region name";
    private static final String TAG = "MapDownloader";

    public void downloadTilesForRegion(Context context, String regionName, int latNE, int longNE, int latSW, int longSW) {

        // Create a bounding box for the offline region
        LatLngBounds latLngBounds = new LatLngBounds.Builder()
                .include(new LatLng(latNE, longNE)) // Northeast
                .include(new LatLng(latSW, longSW)) // Southwest
                .build();

        // Define offline tiles and metadata
        OfflineTilePyramidRegionDefinition definition =
                new OfflineTilePyramidRegionDefinition(
                        context.getResources().getString(R.string.mapbox_style), latLngBounds, 10, 20,
    ...
    private byte[] createMetadata(String regionName) {
        try {
            ...
            Log.e("MapboxBaseFragment", "downloadTilesForRegion: Failed to encode metadata.", exception);
            return null;
        }
    }
